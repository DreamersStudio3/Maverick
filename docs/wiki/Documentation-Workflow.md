...
| 위치 | 역할 | 편집 방식 |
| --- | --- | --- |
| `docs/todo/<task>.md` | 진행 중인 작업의 목표, 상태, 결정, 검증 | 작업자가 직접 갱신하고 완료하면 삭제하는 단기 기록 |
...
2. `docs/todo/<task>.md`에 목표와 초기 체크리스트를 만든다.
...
1. `docs/todo/`의 작업 상태와 미해결 항목을 확인한다. 완료한 작업 파일은 장기 지식을 위키나 헤더에 옮긴 뒤 삭제한다.
