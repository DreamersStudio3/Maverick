---
source_file: "MaverickDesign/CsvToJsonConverter/CsvToJsonConverter.py"
type: "code"
community: "knowledge_guard.py"
location: "L120"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/knowledge_guardpy
---

# convert_client_layout()

## Connections
- [[Any]] - `references` [EXTRACTED]
- [[CsvToJsonConverter.py]] - `contains` [EXTRACTED]
- [[convert_rows()]] - `calls` [EXTRACTED]
- [[find_client_row()]] - `calls` [EXTRACTED]
- [[is_empty_row()]] - `calls` [EXTRACTED]
- [[normalize_header()]] - `calls` [EXTRACTED]
- [[parse_value()]] - `calls` [EXTRACTED]
- [[to_int_flag()]] - `calls` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/knowledge_guardpy