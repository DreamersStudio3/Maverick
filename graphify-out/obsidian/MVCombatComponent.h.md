---
source_file: "Source/Maverick/Components/MVCombatComponent.h"
type: "code"
community: "FMVSkillEntry"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/FMVSkillEntry
---

# MVCombatComponent.h

## Connections
- [[FMVCombatActionEvent]] - `contains` [EXTRACTED]
- [[FMVCombatHeavyChargeAttackRuntimeState]] - `contains` [EXTRACTED]
- [[FMVSkillActionStruct]] - `contains` [EXTRACTED]
- [[FMVSkillEntry]] - `contains` [EXTRACTED]
- [[FMVSkillSlotRuntimeState]] - `contains` [EXTRACTED]
- [[MAVERICK_API_4]] - `contains` [EXTRACTED]
- [[MVActionInputHandlerInterface.h]] - `imports` [EXTRACTED]
- [[MVCombatActionTableInput.h]] - `imports` [EXTRACTED]
- [[MVCombatActionTypes.h]] - `imports` [EXTRACTED]
- [[MVSkillDataTableColumn.h]] - `imports` [EXTRACTED]
- [[UChooserTable]] - `contains` [EXTRACTED]
- [[UMVAbilityBase_3]] - `contains` [EXTRACTED]
- [[UMVStatComponent_2]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/FMVSkillEntry