...
`CombatComponent`와 `HitReactionComponent`는 각자 ChooserTable을 평가해 상황별 액션 테이블 이름, 실행할 `RowName`, 시작 섹션을 확정한 뒤 `ActionComponent`에 전달한다.

상황별 액션 테이블은 런타임 중 동적으로 늘어나지 않고 초기화 시점에 정해진 테이블들이므로, `ActionComponent`에는 `FName ActionTableName`을 넘겨 `MVTableManager`의 manifest 기반 조회를 사용한다.
Chooser 결과는 도메인에 따라 `FDataTableRowHandle` 또는 등록된 테이블 이름 + `RowName`으로 확정하고, `ActionComponent`에는 manifest 기반 테이블 이름을 넘긴다.
HitReaction 테이블은 CSV 생성 체인에서 빼고 `Content/Table/HitReaction/P1`처럼 도메인/캐릭터별 uasset DataTable로 직접 관리한다.

## 전환 원칙

- [x] 이전 중앙 액션 인덱스/스탯 테이블 기반 실행 흐름은 제거하고, Dodge/Sprint도 도메인별 Chooser/DataTable row 흐름으로 전환한다.
- [x] 새 Combat/HitReaction 흐름에서는 `ActionComponent`가 선택 정책을 갖지 않고, 전달받은 테이블 이름과 `RowName`으로 row를 읽어 몽타주 재생과 이벤트 브로드캐스트만 수행한다.
...
- [x] Chooser 결과로 `HR_P1_SH_F` 같은 상황별 액션 테이블 이름을 얻는다.
- [x] P1 HitReaction 테이블은 실제 몽타주 방향만 유지한다.
  - SmallHit: F/L/R/B
  - LargeHit: F
  - KnockDown: F/B
  - Airborne: F/B
- [x] 얻은 테이블에서 실행할 `RowName`, `SectionName`, 몽타주/HitReaction 전용 row 정보를 확정한다.
...
- [x] 이동 입력은 HitReactionComponent가 이벤트로 직접 소비하지 않고, Recovery window가 열리는 순간 InputManager에 저장된 최근 이동 입력을 조회해 SH/LH는 현재 HitReaction 취소, KD/AB는 별도 `Getup` recovery 액션으로 소비한다.
...
- [x] KnockDown/Airborne은 Recovery window 안에 저장된 입력이 없으면 별도 `Getup` recovery 액션으로 전환한다.
- [x] KD/AB 본 리액션은 상태 표현까지만 담당하고, Recovery window에서는 별도 Getup/EscapeDodge row로 전환한다.
