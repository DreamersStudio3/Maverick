 ...
 *
 * CharacterIndexCode는 CharacterIndex의 row key와 같은 GameplayTag로 관리하며, 런타임에서는 이 값을
...
struct MAVERICK_API FMVCharacterStatRow : public FMVTableRowBase
{
	...
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Maverick|Table|Stat", meta = (Categories = "Character"))
	...
	float GetRecentDamageResetDelay() const
	{
		return RecentDamageResetDelay > 0.0f ? RecentDamageResetDelay : GroggyRecoveryDelay;
	}
