...
- `CombatComponent`는 현재 장비 스타일과 전투 프로필을 기준으로 기존 액션/스킬 테이블 선택 흐름을 유지한다.
...
- `Item.Weapon.OneHand.TestSword`
- `Item.Weapon.TwoHand.TestGreatSword`
...
이번 vertical slice에서 무기는 장착 스타일과 공통 공격력만 제공하고, 개별 공격/스킬의 어빌리티와 수치 프로퍼티는 기존 `FMVSkillDataTableColumn` row가 계속 소유한다.
...
- 장착 변경 시 캐릭터의 `EquippedStyle`을 갱신한다.
...
- 이후 고유 무기 액션 세트가 필요하면 `CombatProfileTag`가 아니라 무기 `ItemTag` 또는 무기 정의의 액션/스킬 참조를 `CombatComponent` 입력으로 전달하는 구조를 검토한다.
...
- [ ] 테스트 양손 무기 장착 시 `EquippedStyle`이 바뀌는지 확인
...
- 한손/양손 스타일 변경 시 기존 액션 테이블 선택 흐름을 사용할 수 있다.
