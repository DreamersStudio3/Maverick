---
source_file: "Source/Maverick/Character/PC/Dodge/MVPlayerDodge.h"
type: "code"
community: "FVector"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/FVector
---

# MVPlayerDodge.h

## Connections
- [[AMVCharacterBase_2]] - `contains` [EXTRACTED]
- [[AMVPlayerCharacter_1]] - `contains` [EXTRACTED]
- [[CharacterLocomotionEnums.h]] - `imports` [EXTRACTED]
- [[FMVDodgeInputContext]] - `contains` [EXTRACTED]
- [[MAVERICK_API_2]] - `contains` [EXTRACTED]
- [[MVActionInputHandlerInterface.h]] - `imports` [EXTRACTED]
- [[MVMovementActionTableTypes.h]] - `imports` [EXTRACTED]
- [[UDataTable_2]] - `contains` [EXTRACTED]
- [[UMVActionComponent_4]] - `contains` [EXTRACTED]
- [[UMVInputManagerComponent_1]] - `contains` [EXTRACTED]
- [[UWorld_7]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/FVector