# FMVStartActionCooldownTaskInstanceData

> 22 nodes · cohesion 0.10

## Key Concepts

- **FMVStartActionCooldownTaskInstanceData** (9 connections) — `Source/Maverick/AI/Task/MVStartActionCooldownTask.h`
- **EnterState** (6 connections) — `Source/Maverick/AI/Task/MVStartActionCooldownTask.h`
- **ResolveStartCooldownOwner()** (5 connections) — `Source/Maverick/AI/Task/MVStartActionCooldownTask.cpp`
- **FMVStartActionCooldownTask** (4 connections) — `Source/Maverick/AI/Task/MVStartActionCooldownTask.h`
- **MVStartActionCooldownTask.cpp** (3 connections) — `Source/Maverick/AI/Task/MVStartActionCooldownTask.cpp`
- **MVStartActionCooldownTask.h** (3 connections) — `Source/Maverick/AI/Task/MVStartActionCooldownTask.h`
- **FStateTreeExecutionContext** (2 connections)
- **.GetInstanceDataType()** (2 connections) — `Source/Maverick/AI/Task/MVStartActionCooldownTask.h`
- **APawn** (1 connections)
- **EStateTreeRunStatus** (1 connections)
- **FStateTreeTransitionResult** (1 connections)
- **TObjectPtr** (1 connections)
- **ActionId** (1 connections) — `Source/Maverick/AI/Task/MVStartActionCooldownTask.h`
- **bRequireStartedAction** (1 connections) — `Source/Maverick/AI/Task/MVStartActionCooldownTask.h`
- **Owner** (1 connections) — `Source/Maverick/AI/Task/MVStartActionCooldownTask.h`
- **StartedActionRowName** (1 connections) — `Source/Maverick/AI/Task/MVStartActionCooldownTask.h`
- **StartedActionTableName** (1 connections) — `Source/Maverick/AI/Task/MVStartActionCooldownTask.h`
- **APawn** (1 connections)
- **FName** (1 connections)
- **FStateTreeTaskCommonBase** (1 connections)
- **TObjectPtr** (1 connections)
- **UStruct** (1 connections)

## Relationships

- No strong cross-community connections detected

## Source Files

- `Source/Maverick/AI/Task/MVStartActionCooldownTask.cpp`
- `Source/Maverick/AI/Task/MVStartActionCooldownTask.h`

## Audit Trail

- EXTRACTED: 48 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*