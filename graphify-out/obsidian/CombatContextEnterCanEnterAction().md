---
source_file: "Source/Maverick/AI/Condition/MVCombatActionEnterCondition.cpp"
type: "code"
community: "AI 전투 진입 조건"
location: "L83"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/AI___
---

# CombatContextEnterCanEnterAction()

## Connections
- [[CombatContextEnterIsDistanceInRange()]] - `calls` [EXTRACTED]
- [[CombatContextEnterPassesCommonGate()]] - `calls` [EXTRACTED]
- [[FMVCombatActionEnterConditionInstanceData]] - `references` [EXTRACTED]
- [[MVCombatActionEnterCondition.cpp]] - `contains` [EXTRACTED]
- [[TestCondition_1]] - `calls` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/AI___