---
source_file: "Source/Maverick/Character/PC/Dodge/MVPlayerDodge.cpp"
type: "code"
community: "플레이어 회피 방향 해석"
location: "L92"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/___
---

# DodgeClampControllerSpaceInput()

## Connections
- [[DodgeResolveControllerSpaceInputFromWorldDirection()]] - `calls` [EXTRACTED]
- [[FVector2D_4]] - `references` [EXTRACTED]
- [[MVPlayerDodge.cpp]] - `contains` [EXTRACTED]
- [[UMVPlayerDodgeCacheControllerSpaceMovementInput()]] - `calls` [EXTRACTED]
- [[UMVPlayerDodgeMakeDodgeInputContext()]] - `calls` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/___