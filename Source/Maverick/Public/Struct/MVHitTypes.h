 ...
 * 공격자/피격자/액션 ID를 넘긴다. 정식 흐름에서는 WeaponComponent가 항상 현재 무기
...
struct MAVERICK_API FMVHitResolveRequest
{
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Maverick|Hit")
	int32 ActionId = MVActionIds::None;
 ...
 *
 * 충돌 컴포넌트는 공격자, 피격자, 액션 id만 넘기고, 이 구조체는 HitResolver가 공격자/피격자
 * 스탯과 액션 스탯을 조회해 확정한 HP 피해량, 피격 반응 유형, 후속 컴포넌트가 참고할 문맥을 담는다.
 */
USTRUCT(BlueprintType)
struct MAVERICK_API FMVResolvedHitData
{
	...
	UPROPERTY(BlueprintReadOnly, Category = "Maverick|Hit")
	int32 AttackerCharacterIndexId = 0;

	UPROPERTY(BlueprintReadOnly, Category = "Maverick|Hit")
	int32 VictimCharacterIndexId = 0;
