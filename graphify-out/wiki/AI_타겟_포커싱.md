# AI 타겟 포커싱

> 26 nodes · cohesion 0.11

## Key Concepts

- **FMVFocusingTaskInstanceData** (11 connections) — `Source/Maverick/AI/Task/MVFocusingTask.h`
- **FMVFocusingTask** (6 connections) — `Source/Maverick/AI/Task/MVFocusingTask.h`
- **EnterState** (6 connections) — `Source/Maverick/AI/Task/MVFocusingTask.h`
- **MVFocusingTask.cpp** (5 connections) — `Source/Maverick/AI/Task/MVFocusingTask.cpp`
- **MVFocusingTaskResolveAIController()** (5 connections) — `Source/Maverick/AI/Task/MVFocusingTask.cpp`
- **FStateTreeExecutionContext** (4 connections)
- **ExitState** (4 connections) — `Source/Maverick/AI/Task/MVFocusingTask.h`
- **Tick** (4 connections) — `Source/Maverick/AI/Task/MVFocusingTask.h`
- **MVFocusingTask.h** (4 connections) — `Source/Maverick/AI/Task/MVFocusingTask.h`
- **AAIController** (2 connections) — `Source/Maverick/AI/Task/MVFocusingTask.h`
- **EStateTreeRunStatus** (2 connections)
- **FStateTreeTransitionResult** (2 connections)
- **.GetInstanceDataType()** (2 connections) — `Source/Maverick/AI/Task/MVFocusingTask.h`
- **AAIController** (1 connections)
- **APawn** (1 connections)
- **bCanFocus** (1 connections) — `Source/Maverick/AI/Task/MVFocusingTask.h`
- **bFocusApplied** (1 connections) — `Source/Maverick/AI/Task/MVFocusingTask.h`
- **FocusController** (1 connections) — `Source/Maverick/AI/Task/MVFocusingTask.h`
- **Owner** (1 connections) — `Source/Maverick/AI/Task/MVFocusingTask.h`
- **Target** (1 connections) — `Source/Maverick/AI/Task/MVFocusingTask.h`
- **AActor** (1 connections)
- **APawn** (1 connections)
- **FStateTreeTaskCommonBase** (1 connections)
- **TObjectPtr** (1 connections)
- **UPROPERTY** (1 connections)
- *... and 1 more nodes in this community*

## Relationships

- No strong cross-community connections detected

## Source Files

- `Source/Maverick/AI/Task/MVFocusingTask.cpp`
- `Source/Maverick/AI/Task/MVFocusingTask.h`

## Audit Trail

- EXTRACTED: 70 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*