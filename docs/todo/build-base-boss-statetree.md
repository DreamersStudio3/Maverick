...
- 상태: 5개 상태와 기본 Linked Asset 구성, 공격 쿨다운 Parameter·Global Task 연결 완료; 임시 `K` 전투 시작·`L` 타겟 지정 C++ 경로 구현 및 컴파일 완료; `Dead` 최초 실행 후 StateTree 종단 전환 구성 완료, Editor 재시작 후 PIE 검증 필요
- 결정: 최상위 주요 상태 5개만 유지; Parameters는 보스별 튜닝 값, Global Task는 지속 감지·런타임 상태, 보스별 행동은 Linked Asset 소유
- 위키 검토: `갱신: docs/wiki/Features/AI-StateTree/document.md`; 이번 `Dead` 에셋 내부 종단 전환은 기존 책임·구조 변화가 없어 추가 갱신 불필요
...
- 구현: `Dead`의 `OnStateCompleted` 대상을 `Root`에서 `Tree Succeeded`로 변경하여 다른 상태 재진입 차단
...
- 검증 통과: Unreal Editor에서 `Dead` 전환 `OnStateCompleted -> Succeeded`, `STT_Dead`의 `EnterState -> FinishTask(false)` 및 StateTree 컴파일 성공 로그 확인
