---
source_file: "Source/Maverick/Components/MVCombatComponent.h"
type: "code"
community: "FMVCombatActionEvent"
location: "L35"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/FMVCombatActionEvent
---

# FMVCombatActionEvent

## Connections
- [[AActor_17]] - `references` [EXTRACTED]
- [[ActionIndex]] - `defines` [EXTRACTED]
- [[ActionRowName]] - `defines` [EXTRACTED]
- [[ActionTableName]] - `defines` [EXTRACTED]
- [[ActionType_1]] - `defines` [EXTRACTED]
- [[EMVCombatActionTypes_2]] - `references` [EXTRACTED]
- [[FName_36]] - `references` [EXTRACTED]
- [[Instigator]] - `defines` [EXTRACTED]
- [[MVCombatComponent.h]] - `contains` [EXTRACTED]
- [[MVDodgeThreatResolveActionTag()]] - `references` [EXTRACTED]
- [[TObjectPtr_26]] - `references` [EXTRACTED]
- [[UMVDodgeThreatEvaluatorLibraryEvaluateDodgeThreat()]] - `references` [EXTRACTED]
- [[UMVEnemyDodgeTokenComponentTryConsumeDodgeTokenForThreat()]] - `references` [EXTRACTED]
- [[int32_17]] - `references` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/FMVCombatActionEvent