...
4. 오래 남을 책임, 흐름, 설계 이유, 에셋 연결에 변화가 있으면 개발자 또는 작업 에이전트가 기존 문서 갱신
5. 기존 문서로 다루기 어려운 독립 주제만 새 문서로 추가
6. 사람이 Obsidian에서 변경 문서와 실제 작업 결과를 대조
7. push 전 TODO에 `갱신: docs/wiki/<문서>.md` 또는 `변경 불필요: <이유>` 기록
8. PR 생성 시 같은 결과를 PR 설명의 `위키 검토` 항목으로 이동
...
- 사람과 작업 에이전트가 편집하고 사람이 최종 검토하는 위치: `docs/wiki/`
